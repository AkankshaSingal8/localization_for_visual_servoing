#!/usr/bin/env python3
"""Generate tight, transparent-background PNG cutouts of boxes using SAM2.

For each image in `objects/`, prompts SAM2 with a small cluster of positive
points near the image centre (where the box is always photographed) and four
negative points at the image corners (which are always background).  SAM2
returns multiple candidate masks; we pick the one whose connected component
contains the image centre and has the best score.

Outputs:
    masked_objects/<name>.png   RGBA, background alpha=0, cropped to bbox
    masked_objects/debug/<name>_debug.png   (optional) overlay for QA

Usage:
    conda run -n semvs python create_masks.py
    conda run -n semvs python create_masks.py --no-crop --debug
"""
...
def center_prompts(h: int, w: int, y_spread: float):
    """Positive points spread vertically by `y_spread` (fraction of image h)
    around the image centre, with horizontal neighbours and corner negatives.

    A larger `y_spread` helps SAM2 group tall boxes that span the frame; a
    smaller spread keeps all positive points on the object when it's small.
    """
    cx, cy = w // 2, h // 2
    dx = w // 8
    half = y_spread * h / 2.0
...
def center_box_prompt(h: int, w: int, x_frac: float, y_frac: float) -> np.ndarray:
    """Loose bounding box covering the region where the object is expected.

    Margins are expressed as fractions of image width/height.  A tighter box
    (larger margins) helps when the object is small; a looser box helps when
...
def largest_component_containing(mask: np.ndarray, point: tuple[int, int]) -> np.ndarray:
    ...
    if target == 0:
        # Centre fell on background — take largest non-background component.
...
def refine_mask(mask: np.ndarray) -> np.ndarray:
    """Close small gaps, fill holes, keep a single clean blob."""
    ...
    # Fill internal holes so the alpha channel is fully opaque inside the box.
...
def strip_backdrop_leaks(
    ...
    """Remove true backdrop leaks (black cloth / grey shadow) from the mask.

    A "backdrop" pixel is dark (HSV V < `v_thresh`) and mostly grey
    (S < `s_thresh`).  We find connected components of such pixels across
    the whole image and drop a component's inside-mask portion only when
    all of the following hold:

    1. The component spans both inside and outside the mask.
    2. The outside portion reaches the image frame (real cloth always does).
    3. The inside-mask portion is no bigger than `max_drop_frac` of the
       current mask area.  This is the critical guard against eroding
       large dark-but-coloured box panels (e.g. the navy right half of
       the TRUBAR protein-bar box) whose pixels happen to satisfy the
       backdrop threshold near boundaries.  Real cloth leaks are small;
       whole-panel "leaks" are false positives.
    """
    ...
    # Small closing bridges thin gaps (e.g. text overlays or fabric highlights)
    # so a contiguous drape + its small intrusion form one CC.
    ...
    for cc_id in range(1, n_cc):
        ...
        if inside > max_drop_frac * mask_area:
            # Too big to be a leak — likely a legitimate dark box panel
            # (e.g. TRUBAR navy half) that happens to fall below the
            # HSV backdrop threshold.
            continue
...
def _run_single_prompt(
    ...
    """Run SAM2 with one prompt config; return (valid-mask-or-None, score)."""
    ...
    for m, s in zip(masks, scores):
        ...
        # Reject degenerate picks: empty, a tiny speck, or the whole frame
        # (which would mean SAM2 grabbed the background).
...
def predict_box_mask(
    ...
    """Run SAM2 with several prompt configurations; return (mask, score, cfg).

    Boxes in this dataset range from tight close-ups that fill the frame to
    small objects sitting on a table with a lot of backdrop around them.  A
    single prompt spread does not work for both — so we try a few and keep
    whichever candidate has the highest SAM2 score among valid masks.
    """
    ...
    if best_mask is None:
        # Fallback: no valid mask across all configs — take the wide config's
        # top raw mask, even if degenerate, so we still write something.
