#!/usr/bin/env python3
"""
Record top-face corners of an object via xArm TCP hover, then solve Horn's
method to produce a ground-truth object pose in the robot base frame.

Independent of the ZED, FoundationPose, and Depth Anything V2: the only
sensor involved is the xArm's joint encoders.

Usage:
    python ground_truth/record_corners.py [--object NAME] [--ip IP]

See ground_truth/README.md for the full procedure.
"""
...
def body_frame_corners(dims_mm):
    """
    Top-face corners in the object body frame. Origin = box centroid.
    X along length, Y along width, Z along height (up). Ordered clockwise
    starting at (+L/2, +W/2, +H/2) when looking down -Z.
    """
    L, W, H = dims_mm
    return np.array([
        [+L/2, +W/2, +H/2],   # C1
...
def horn_rigid_fit(body_pts, base_pts):
    """Solve R, t such that base = R @ body + t (rigid, least squares)."""
...
def report_fit(dims_mm, base_corners, residuals):
    ...
    # Edge-length sanity check (order-independent: multiset compare).
...
def save_output(obj_name, dims_mm, R, t, body_corners, base_corners,
    ...
    # The body-frame origin is the box centroid by construction, so the
    # Horn-fit translation t IS the object centroid in base frame.
    # (NOT the mean of top-face corners, which sits H/2 above the centroid.)
    centroid = t
...
def main():
    ...
    while True:
        ...
        if rms > RESIDUAL_WARN_MM or edge_err > RESIDUAL_WARN_MM:
            print(f"\n⚠️  Residual ({rms:.2f} mm) or edge error ({edge_err:.2f} mm) "
