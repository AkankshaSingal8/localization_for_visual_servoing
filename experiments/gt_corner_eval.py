#!/usr/bin/env python3
"""
Ground-truth corner evaluation for saved vs_dinov2 videos.

For each vs_dinov2_{object}_{controller}_{date}_{time}.mp4 in the repo root:
  1. Matches it to the corresponding metrics CSV in runs/
  2. Per frame: determines GT bounding-box corners of the target box using one
     of two methods, whichever fires first:
       Method A (green mask)  — extracts the SAM2 green-channel overlay that
         dinov2_servo.py blends at 28 % opacity; used for ekf/pbvs frames where
         the mask is visible.
       Method B (edge/Hough)  — runs Canny + HoughLinesP inside a tight ROI
         centred on the pipeline centroid; used for foundationpose frames (no
         SAM2 mask overlay) and as a fallback for ekf/pbvs frames where the
         mask is absent.
  3. Per frame: computes pixel error between the pipeline centroid (from CSV)
     and the GT center derived from detected corners.
  4. Writes an annotated video overlaying GT corners (yellow), GT center (green
     crosshair), servo centroid (red dot), and an error line (white).
  5. Writes a per-frame error CSV and a summary CSV.

Usage:
    python experiments/gt_corner_eval.py
    python experiments/gt_corner_eval.py \\
        --video vs_dinov2_cheez_it_box_fp_20260419_211930.mp4
    python experiments/gt_corner_eval.py --out-dir experiments/gt_eval
"""
import argparse
import csv
import math
...
# ── Method A: SAM2 green-mask detection ─────────────────────────────────
# dinov2_servo.py blends the SAM2 binary mask as:
#   G_out = 0.72*G_orig + 0.28*255  (overlay channel 1 set to 255 for mask pixels)
# This lifts the G channel by ~71 counts in the mask region, making G > R and
# G > B detectable with the threshold below.
GREEN_EXCESS_THRESH = 12    # G must exceed both R and B by at least this
GREEN_MORPH_K       = 9     # morphological kernel size (px) for cleanup
MASK_MIN_AREA       = 2500  # minimum contour area (px²)  — 50×50
MASK_MAX_AREA_FRAC  = 0.25  # reject masks covering > 25 % of frame area

# ── Method B: edge/Hough detection ──────────────────────────────────────
EDGE_ROI_RADIUS     = 280   # half-side of the ROI around the centroid hint (px)
EDGE_CANNY_LO       = 40
EDGE_CANNY_HI       = 120
EDGE_HOUGH_THRESH   = 25    # votes needed for a Hough line
EDGE_MIN_LINE_FRAC  = 0.12  # minimum line length as fraction of ROI width

# ── Common geometry sanity checks ────────────────────────────────────────
BOX_MIN_DIM      = 40    # minimum detected side length (px)
BOX_MAX_DIM_FRAC = 0.80  # maximum side as fraction of frame dimension
BOX_MAX_ASPECT   = 5.0   # reject very elongated rectangles
...
# ── CSV matching & loading ───────────────────────────────────────────────

def find_matching_csv(object_name: str, controller: str,
                      date_str: str, video_unix: float) -> Path | None:
    """
    Match a video to its metrics CSV in runs/.

    Primary search: runs/{object_name}_{date}_*/{object_name}_{controller}.csv
    Fallback (protein_bar / protein_bar_2 split sessions): if no match is
    found and the object name ends with '_2', retry without the suffix (and
    vice versa). This handles the case where the EKF, FP, and PBVS trials for
    a single physical object were recorded in separate sessions saved under
    different names (protein_bar_ekf.csv vs protein_bar_2_fp.csv).
    Note: timestamp alignment is preserved — the CSV must be from the same
    calendar date, and the closest-timestamp directory wins.
    """
...
def nearest_centroid(lookup: list, query_ts: float):
    """Binary-search lookup for the CSV row nearest to query_ts."""
    if not lookup:
        return None, None
...
def _cluster_split(vals: list) -> tuple:
    """Split a sorted list into two clusters at the largest gap."""
    if len(vals) < 2:
        return vals, []
...
# ── Method A: SAM2 green-mask ────────────────────────────────────────────

def _detect_from_green_mask(frame_bgr: np.ndarray,
                             hint_point: tuple | None) -> np.ndarray | None:
    """
    Extract the SAM2 green overlay region and fit an axis-aligned bounding
    rectangle.  Returns corners [TL,TR,BR,BL] or None.
    """
...
# ── Method B: edge / Hough in ROI ────────────────────────────────────────

def _detect_from_edges(frame_bgr: np.ndarray,
                        hint_point: tuple) -> np.ndarray | None:
    """
    Within a tight ROI around hint_point, detect the dominant rectangular box
    via Canny edge detection + HoughLinesP.  Horizontal and vertical line
    midpoints are split into two clusters each (top/bottom, left/right), and
    the median of each cluster defines one side of the bounding rectangle.
    Returns corners [TL,TR,BR,BL] in full-frame coordinates, or None.
    """
    fh, fw = frame_bgr.shape[:2]
    ...
    for ln in lines:
        ...
        if dx == 0 and dy == 0:
            continue
        if dy < 0.25 * dx:           # near-horizontal
            h_mids.append((ly1 + ly2) / 2.0)
        elif dx < 0.25 * dy:          # near-vertical
    ...
    # Sort and convert back to full-frame coordinates
...
# ── Unified per-frame GT detection ───────────────────────────────────────
...
def detect_box_corners(frame_bgr: np.ndarray,
    ...
    """
    Try Method A (green mask) first; fall back to Method B (edge/Hough) when
    hint_point is available.  Pass edge_only=True to skip Method A entirely.

    Returns (corners_or_None, method_string) where method_string is one of
    'mask', 'edge', or 'none'.
    """
    if not edge_only:
...
def draw_overlays(frame: np.ndarray,
    ...
    if centroid_u is not None and centroid_v is not None:
        if use_cam_center:
            # Camera centre used as FP reference — blue crosshair marker
            arm = 12
            ...
            # Tracked servo centroid — red filled circle
...
def process_video(video_path: Path, out_dir: Path, verbose: bool = True,
    ...
    # All controllers are evaluated against the camera centre (image centre)
    # as the reference point — this measures how close the object actually
    # got to where the servo is trying to drive it, independent of which
    # pipeline was used.  The CSV centroid is still loaded and used as a
    # spatial hint for Method B edge detection, but not for error calculation.
    cam_cx = width  // 2
    cam_cy = height // 2
    if verbose:
    ...
    frame_idx = 0
    last_gt_center: tuple | None = None   # carry-forward for hint-free frames
    while True:
        ret, frame = cap.read()
        if not ret:
            break
        ...
        # CSV centroid used only as a spatial hint for edge detection (Method B).
        ...
        # Error reference is always the camera centre for all controllers.
        cu, cv_ = cam_cx, cam_cy

        # Edge-detection hint fallback when CSV has no centroid.
        if hint is None:
