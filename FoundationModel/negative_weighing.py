...
def _robust_centroid(mask: np.ndarray):
    ...
    Handles internal horizontal/vertical lines that can split the mask
    and throw off a naïve moments-based centroid.

    Strategy:
      1. Morphological close to bridge gaps from printed lines / textures.
      2. Find the largest external contour.
      3. Convex hull → fill → minAreaRect centre  (very robust for rectangles).
      4. Verify the centre lies inside the (closed) mask; fall back to
         convex-hull moments otherwise.
    """
    # --- Step 1: close small gaps from internal lines -----------------
...
class MaskTracker:
    """
    Maintains inter-frame state so that SAM2 can propagate a consistent
    mask across the video stream:

    * ``prev_logits``   – low-res (1×256×256) logits from the last
                          SAM2 prediction, fed back as ``mask_input``
    * ``prev_mask``     – full-res binary mask (uint8, 0/255)
    * ``mask_history``  – ring buffer of recent masks used to mine
                          *reliable negative points* once the tracker
                          has warmed up (≥ TRACKER_WARMUP_FRAMES)
    * ``frame_count``   – consecutive frames with a valid mask
    """
    ...
    def sample_negative_points(self, h: int, w: int,
        ...
        """
        After warm-up, return ``n`` (x,y) points that have been
        *consistently outside* every recent mask — safe negatives.
        Returns Nx2 np.float32 array or None.
        """
        if not self.warmed_up:
            return None

        # Accumulate: pixel is "always background" if it was never
        # inside any of the last WARMUP masks
    ...
    def sample_positive_points(self, h: int, w: int,
        ...
        """
        Sample positive point(s) from the interior of the previous mask.
        ...
        if n > len(pts) and self.prev_mask is not None:
            mr = self.prev_mask if self.prev_mask.shape[:2] == (h, w) else \
                cv2.resize(self.prev_mask, (w, h),
                           interpolation=cv2.INTER_NEAREST)
            # Erode so points are well inside the mask
...
def _disambiguate_top_box(dets: list, image_shape: tuple,
    ...
    """
    From a list of (xyxy, score, phrase) detections, pick the single
    *top box* in a scene of identical stacked boxes.

    Pipeline:
      1. NMS to collapse near-duplicate detections.
      2. Filter out implausibly small / large boxes.
      3. Score each surviving detection by a weighted combination of:
         - *vertical position*  (prefer top-of-image  → top of stack)
         - *detector confidence*
         - *proximity to previous centroid*  (temporal smoothness)
      4. Return the best-scoring detection.
    """
    if not dets:
        return None
    ...
    for det in dets:
        ...
        if area_ratio < 0.003 or area_ratio > 0.75:
            continue
        # Reject extremely elongated strips (likely a row of stacked edges)
    ...
    if not filtered:
        # Fall back to simple topmost if filters were too aggressive
    ...
    for det in filtered:
        ...
        # Higher position in image → top of stack  (small y → high score)
        ...
        if prev_centroid is not None:
            ...
            # Normalise so that a distance of ~50 px gives ~0.5 bonus
...
# ═════════════════════════════════════════════════════════════════════
#  Core pipeline  (with mask propagation)
# ═════════════════════════════════════════════════════════════════════

def run_pipeline(image_bgr: np.ndarray, prompt: str,
                 tracker: MaskTracker | None = None) -> dict:
    """
    Run detection → SAM2 segmentation → depth → grasp-point pipeline.

    When *tracker* is provided the function uses SAM2 mask propagation:
      • Previous low-res logits are fed back via ``mask_input``.
      • The previous centroid is used as a positive point prompt.
      • After warm-up, negative points mined from the background are added.
      • The detector is re-run every REDETECT_INTERVAL frames (or when
        the mask IoU drifts below IOU_DRIFT_THRESH) to correct drift.
    """
    ...
    try:
        ...
        if masks is not None and len(masks) > 0:
            ...
            # ── Drift check: if mask changed too much, reset ─────────
...
# ═════════════════════════════════════════════════════════════════════
#  Robot controller  (unchanged from original)
# ═════════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════════
#  Camera streamer  (updated for tracker)
# ═════════════════════════════════════════════════════════════════════
